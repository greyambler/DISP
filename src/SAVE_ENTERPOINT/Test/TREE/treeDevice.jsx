...
export default class treeDevice extends PureComponent {
    ...
    get_Full_TreeBar() {
        ...
        let D_Tree = { 'name': 'Справочники', "key": "Tree_root", "children": new Array(), 'toggled': true };

        for (const ItemDev of J_Rss) {

            let I_Device = {
                'id': ItemDev.id, 'name': ItemDev.nm, 'type': ItemDev.typ,
                "key": "tree_" + ItemDev.typ, "children": this.get_Child(ItemDev.cntyp), 'toggled': true
            };


            D_Tree.children.push(I_Device);
        }
    }

    get_Full_TreeBar_save() {
        let J_Rss = this.state.Objects;//JSON.parse(_Json);
        let IsOnlyAZS = false;
        let D_Tree = null;
    ...
    get_Child(list) {
        ...
        if (list != undefined) {
            for (const item of list) {
                ...
                let ops=new Array();
                if (item.def.op != undefined) {
                    for (const it_OP of item.def.op) {
                        ops.push({ val: it_OP.val, text: it_OP.text });
                    }
                }

                children.push({ type: item.typ, text: item.def.nm, op: ops });
            }
        }
